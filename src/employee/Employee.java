...
class employeemeta {
	...
	void add_meta() throws IOException {
		...
		System.out
				.println("Please enter User name for login..It should be number");
		int uk = 0;
		while (uk != 4) {
			...
			if (s == 0) {
				...
				if (this.type_of_employee == 1) {
					objf.add_infile(us,
							"Designnation:-->Senior Software Engineer");
...
class senior extends employeemeta {
	...
	void calc_cal() {
		gross_sal = basic_sal + ((basic_sal / 100) * da)
				+ ((basic_sal / 100) * hra) + ((basic_sal / 100) * ta);
...
public class Employee {
	...
	public static void main(String[] args) throws IOException {
		...
		while (choice != 4) {
			System.out
					.println("*********************Employee Managment System*********************");
			...
			while (x2 != 1) {
				...
				if (ch > 0) {
					...
					x2 = 0;
					System.out
							.println("Please enter only number not alphabets");
				}
